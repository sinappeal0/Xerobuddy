...
import 'package:xerobuddy_app/screens/patient_side/patientReport/patientReportPreview.dart';

import '../../../model/bottomNavigationBar.dart';
import '../../../util/globals.dart';

class PatientAverageRisk extends StatefulWidget {
  const PatientAverageRisk({super.key});

  @override
  State<PatientAverageRisk> createState() => _PatientAverageRiskState();
}

class _PatientAverageRiskState extends State<PatientAverageRisk> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text("Please enter the email below to send your saliva assessment report.", style: GoogleFonts.poppins(color: Colors.black54),),
                                ...
                                child: ElevatedButton(
                                  ...
                                  onPressed: () async {
                                    ...
                                    try {
                                      ...
                                      FirebaseFirestore.instance.collection("users").doc(patientEmailController.text).collection("reports").doc(newDocumentName.text).set({
                                      ...
                                      FirebaseFirestore.instance.collection("users").doc(currentUser!.email).collection("reports").doc(newDocumentName.text).collection("drugsRecommended").get().then((QuerySnapshot snapshot) {
                                        snapshot.docs.forEach((DocumentSnapshot doc) {
                                          var item = doc.data() as Map<String, dynamic>;
                                          try {
                                            FirebaseFirestore.instance.collection("users").doc(patientEmailController.text).collection("reports").doc(newDocumentName.text).collection("drugsRecommended").doc(doc.id).set(item);
                                          } on FirebaseException catch (e) {
                                            print("=============================================\n\n");
                                            print(e.code);
                                            print("\n\n=============================================");
                                      ...
                                      FirebaseFirestore.instance.collection("users").doc(currentUser!.email).collection("reports").doc(newDocumentName.text).collection("xerostomia_questions").get().then((QuerySnapshot snapshot) {
                                        snapshot.docs.forEach((DocumentSnapshot doc) {
                                          var item = doc.data() as Map<String, dynamic>;
                                          try {
                                            FirebaseFirestore.instance.collection("users").doc(patientEmailController.text).collection("reports").doc(newDocumentName.text).collection("xerostomia_questions").doc(doc.id).set(item);
                                          } on FirebaseException catch (e) {
                                            print("=============================================\n\n");
                                            print(e.code);
                                            print("\n\n=============================================");
      ...
      body: StreamBuilder(
        stream: FirebaseFirestore.instance.collection("users").doc(currentUser!.email).collection("reports").doc(newDocumentName.text).snapshots(),
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(
                      height: 180,
                      child: Image.asset("assets/images/yellow light 1.png"),
                    ),
                    const SizedBox(height: 40,),

                    Text("Average Risk", style: GoogleFonts.poppins(fontWeight: FontWeight.w700, color: Colors.black, fontSize: 28), textAlign: TextAlign.center,),
                    const SizedBox(height: 20,),
                    ...
                    Padding(
                      ...
                      child: ListView.builder(
                        ...
                        shrinkWrap: true,
                        itemCount: summaryDrugList.length,
                        itemBuilder: (context, index) {
                          // Get single summary drugs
                          final drug = summaryDrugList[index];
                      ...
                      child: Row(
                        ...
                        children: [
                          ElevatedButton.icon(
                            ...
                            onPressed: () async {
                              try {
                                Navigator.push(context, MaterialPageRoute(builder: (context) => const PatientReportPreview()));
                              } on FirebaseException catch(e) {
                                print("=============================");
  ...
  List summaryDrugList = [];

  getDrugsRecommended () async {
    await FirebaseFirestore.instance.collection("users").doc(currentUser!.email).collection("reports").doc(newDocumentName.text).collection("drugsRecommended").get().then((value) {
      for(var i in value.docs) {
        summaryDrugList.add(i.data());
        drugsRecommended.add(i.data());
        distinctDrugList = drugsRecommended.toSet().toList();
      }
      print("---------------------------");
      print("Drugs Recommended : ${drugsRecommended}");
    });
  }
}
