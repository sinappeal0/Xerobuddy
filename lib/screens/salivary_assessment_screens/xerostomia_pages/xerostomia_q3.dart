...
import 'package:xerobuddy_app/screens/salivary_assessment_screens/xerostomia_pages/xerostomia_q4.dart';

import '../../../model/bottomNavigationBar.dart';
import '../../../util/globals.dart';

class XerostomiaQ3 extends StatefulWidget {
  const XerostomiaQ3({super.key});

  @override
  State<XerostomiaQ3> createState() => _XerostomiaQ3State();
}

class _XerostomiaQ3State extends State<XerostomiaQ3> {
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Text(xQuestions[0], style: GoogleFonts.poppins(fontSize: 26, fontWeight: FontWeight.w600), textAlign: TextAlign.center,),
            ),
            ...
            GestureDetector(
              onTap: () {
                try {
                  FirebaseFirestore.instance.collection("users").doc(currentUser!.email).collection("reports").doc(newDocumentName.text).collection("xerostomia_questions").doc("Question3").set({
                    "xQuestion": xQuestions[0],
                    "xQuestionAnswer": "Never",
                  });

                  // Redirect to next page
                  Navigator.push(context, MaterialPageRoute(builder: (context) => const XerostomiaQ4()));
                } on FirebaseException catch(e) {
                  ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
