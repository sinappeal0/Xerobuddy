...
import 'package:xerobuddy_app/screens/salivary_assessment_screens/xerostomia_pages/xerostomia.dart';

import '../../../model/bottomNavigationBar.dart';
import '../../../util/globals.dart';

class XerostomiaQ5 extends StatefulWidget {
  const XerostomiaQ5({super.key});

  @override
  State<XerostomiaQ5> createState() => _XerostomiaQ5State();
}

class _XerostomiaQ5State extends State<XerostomiaQ5> {
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Text(xQuestions[2], style: GoogleFonts.poppins(fontSize: 26, fontWeight: FontWeight.w600), textAlign: TextAlign.center,),
            ),
            ...
            GestureDetector(
              onTap: () {
                try {
                  FirebaseFirestore.instance.collection("users").doc(currentUser!.email).collection("reports").doc(newDocumentName.text).collection("xerostomia_questions").doc("Question5").set({
                    "xQuestion": xQuestions[2],
                    "xQuestionAnswer": "Never",
                  });

                  // Redirect to next page
                  Navigator.push(context, MaterialPageRoute(builder: (context) => const Xerostomia()));
                } on FirebaseException catch(e) {
                  ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
