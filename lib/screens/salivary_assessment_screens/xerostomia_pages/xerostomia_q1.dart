...
import 'package:xerobuddy_app/screens/salivary_assessment_screens/xerostomia_pages/xerostomia_q2.dart';

import '../../../model/bottomNavigationBar.dart';
import '../../../util/globals.dart';

class XerostomiaQ1 extends StatefulWidget {
  const XerostomiaQ1({super.key});

  @override
  State<XerostomiaQ1> createState() => _XerostomiaQ1State();
}

class _XerostomiaQ1State extends State<XerostomiaQ1> {
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Text(xQuestions[1], style: GoogleFonts.poppins(fontSize: 26, fontWeight: FontWeight.w600), textAlign: TextAlign.center,),
            ),
            ...
            GestureDetector(
              onTap: () {
                try {
                  FirebaseFirestore.instance.collection("users").doc(currentUser!.email).collection("reports").doc(newDocumentName.text).collection("xerostomia_questions").doc("Question1").set({
                    "xQuestion": xQuestions[1],
                    "xQuestionAnswer": "Never",
                  });

                  // Redirect to next page
                  Navigator.push(context, MaterialPageRoute(builder: (context) => const XerostomiaQ2()));
                } on FirebaseException catch(e) {
                  ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
