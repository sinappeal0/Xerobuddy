...
import 'package:xerobuddy_app/screens/salivary_assessment_screens/xerostomia_pages/xerostomia_q3.dart';

import '../../../model/bottomNavigationBar.dart';
import '../../../util/globals.dart';

class XerostomiaQ2 extends StatefulWidget {
  const XerostomiaQ2({super.key});

  @override
  State<XerostomiaQ2> createState() => _XerostomiaQ2State();
}

class _XerostomiaQ2State extends State<XerostomiaQ2> {
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Text(xQuestions[3], style: GoogleFonts.poppins(fontSize: 26, fontWeight: FontWeight.w600), textAlign: TextAlign.center,),
            ),
            ...
            GestureDetector(
              onTap: () {
                try {
                  FirebaseFirestore.instance.collection("users").doc(currentUser!.email).collection("reports").doc(newDocumentName.text).collection("xerostomia_questions").doc("Question2").set({
                    "xQuestion": xQuestions[3],
                    "xQuestionAnswer": "Never",
                  });

                  // Redirect to next page
                  Navigator.push(context, MaterialPageRoute(builder: (context) => const XerostomiaQ3()));
                } on FirebaseException catch(e) {
                  ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
