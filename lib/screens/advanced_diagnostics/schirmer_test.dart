...
import 'package:xerobuddy_app/screens/advanced_diagnostics/schirmer_test_tut.dart';

import '../../model/bottomNavigationBar.dart';
import '../../util/globals.dart';

class Schirmer_Test extends StatefulWidget {
  const Schirmer_Test({super.key});

  @override
  State<Schirmer_Test> createState() => _Schirmer_TestState();
}

class _Schirmer_TestState extends State<Schirmer_Test> {
  final User? currentUser = FirebaseAuth.instance.currentUser!;

  // Schirmer Test
  TextEditingController schirmer_test = TextEditingController();
      ...
      body: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            GestureDetector(
              onTap: () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => const Schirmer_Test_Tutorial()));
              },
              child: Container(
                height: 55,
                width: 180,
            ...
            ElevatedButton(
              ...
              onPressed: () {
                ...
                FirebaseFirestore.instance.collection("users").doc(currentUser!.email).collection("reports").doc(newDocumentName.text).update({
                  "schirmer": double.parse(schirmer_test.text),
                });
              },
